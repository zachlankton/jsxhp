...
const publicFilesMap = publicFiles.reduce(
	(prevValue, currValue) => {
			...
			return await requestContext.run(
				...
				async () => {
					try {
						...
						let jsxTree: JSX.Element | null = null;
						...
						const stream = new ReadableStream({
							async start(controller) {
								...
								// get system temp folder
								const tempDir =
									Bun.env.TMPDIR ?? Bun.env.TEMPDIR ?? Bun.env.TEMP;
								const tempFilePath = `${tempDir}/${sessionId}.json`;
...
process.on("unhandledRejection", (reason, promise) => {
	...
	// Intentionally NOT calling process.exit() — keep server running
});
